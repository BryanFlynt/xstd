...
struct range_iterator {
	...
	explicit range_iterator(const value_type& value) :
			value_(value),
			step_(1){
	}

	explicit range_iterator(const value_type& value, const value_type& step) :
			value_(value),
			step_(step){
	}
	...
	friend range_iterator operator+(range_iterator x, difference_type y) {
		return x += y * x.step_;
	}

	friend range_iterator operator+(difference_type x, range_iterator y) {
		return y += x * y.step_;
	}
...
#endif /* INCLUDE_XSTD_ITERATOR_RANGE_HPP_ */
