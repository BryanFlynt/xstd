...
#ifndef INCLUDE_XSTD_ITERATOR_ZIP_HPP_
#define INCLUDE_XSTD_ITERATOR_ZIP_HPP_


#include "xstd/tuple.hpp" // xstd::for_each
...
struct zip_iterator {
	...
	zip_iterator& operator++(){
		xstd::for_each(iterators_, [](auto& iter){
			iter++;
		});
		return *this;
	}
	...
	zip_iterator& operator+=(const difference_type& inc){
		xstd::for_each(iterators_, [inc](auto& iter){
			iter += inc;
		});
		return *this;
	}
	...
	value_tuple operator[](const difference_type n) {
		return xstd::transform(iterators_, [n](auto iter) -> decltype(*iter)& {
			return iter[n];
		});
	}

	reference_tuple operator*(){
		return xstd::transform(iterators_, [](auto iter) -> decltype(*iter)& {
			return *iter;
		});
	}
	...
	friend bool operator<(const zip_iterator& x, const zip_iterator& y) {
		return xstd::all_of(x.iterators_,y.iterators_,[](const auto& a, const auto& b){
			return (a < b);
		});
	}

	friend difference_type operator-(const zip_iterator& x, const zip_iterator& y) {
		auto diff_tuple = xstd::transform(x.iterators_,y.iterators_,[](const auto& a, const auto& b){
			return a - b;
		});
		return xstd::min(diff_tuple); // Return minimum distance
	}

	friend zip_iterator operator+(zip_iterator x, difference_type y) {
		return x += y;
	}

	friend zip_iterator operator+(difference_type x, zip_iterator y) {
		return y += x;
	}
...
struct zip_proxy {
	 ...
	 */
	zip_proxy(const ZipIterator& first, const ZipIterator& last) :
		first_(first),
		last_(last){
	}
...
#endif /* INCLUDE_XSTD_ITERATOR_ZIP_HPP_ */
